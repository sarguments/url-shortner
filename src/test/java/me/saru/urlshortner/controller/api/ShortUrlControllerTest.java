...
class ShortUrlControllerTest extends DefaultSpringTest<ShortUrl> {
    ...
    void checkUrlByHibernateValidator() throws Exception {
        mockMvc.perform(post("/api/shorturl")
                ...
                .content(convertToJson(new ShortUrlDto.Req("test")))
        )
                .andDo(print())
        ...
        mockMvc.perform(post("/api/shorturl")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
                .content(convertToJson(new ShortUrlDto.Req("test.com")))
        )
                .andDo(print())
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success", is(false)));
        ...
        mockMvc.perform(post("/api/shorturl")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
                .content(convertToJson(new ShortUrlDto.Req("http://test.com")))
        )
                .andDo(print())
    ...
    void generateShortUrlEmpty() throws Exception {
        mockMvc.perform(post("/api/shorturl")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
                .content(convertToJson(new ShortUrlDto.Req("")))
        )
                .andDo(print())
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success", is(false)))
